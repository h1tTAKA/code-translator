          ...
          <div className="flex flex-col gap-3">
            ...
            {editingBody ? (
              <div className="flex flex-col gap-1">
                ...
                <div className="flex justify-end gap-1">
                  ...
                  <button type="button" onClick={() => void runAct(() => window.nunopiDesktop!.github!.editBody(root, "issue", number, bodyDraft.trim()))} disabled={actBusy || !bodyDraft.trim()} className="inline-flex items-center gap-1 rounded-md bg-zinc-800 px-2.5 py-1 text-[11px] font-medium text-white hover:bg-zinc-700 disabled:opacity-40 dark:bg-zinc-200 dark:text-zinc-900 dark:hover:bg-white">{actBusy && <IconLoader2 size={11} className="animate-spin" aria-hidden />}{t("github.save")}</button>
                </div>
              </div>
            ) : (
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              {d.state.toUpperCase() === "OPEN"
                ? <button type="button" onClick={() => void runAct(() => window.nunopiDesktop!.github!.setState(root, "issue", number, "close"))} disabled={actBusy} className="inline-flex items-center gap-1 rounded-md border border-zinc-200 px-2 py-1 text-[11px] font-medium text-zinc-600 transition hover:bg-zinc-100 disabled:opacity-40 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800">{actBusy && <IconLoader2 size={11} className="animate-spin" aria-hidden />}{t("github.close")}</button>
                : <button type="button" onClick={() => void runAct(() => window.nunopiDesktop!.github!.setState(root, "issue", number, "reopen"))} disabled={actBusy} className="inline-flex items-center gap-1 rounded-md border border-zinc-200 px-2 py-1 text-[11px] font-medium text-zinc-600 transition hover:bg-zinc-100 disabled:opacity-40 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800">{actBusy && <IconLoader2 size={11} className="animate-spin" aria-hidden />}{t("github.reopen")}</button>}
