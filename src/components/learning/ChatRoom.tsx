    ...
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-[#111219]">
      ...
      <div ref={scrollRef} className="nunopi-scroll min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
        ...
        {messages.map((m, i) => {
          if (m.role === "user") {
            return (
              <div key={i} className="flex justify-end">
                <div className={`max-w-[85%] select-text whitespace-pre-wrap rounded-2xl bg-zinc-100 px-3 py-2 text-zinc-800 dark:bg-zinc-700 dark:text-zinc-100 ${userTxt}`}>
                  {m.content}
                </div>
              </div>
            );
          ...
          // 이미 카드로 있는 용어도 칩은 띄우되 "이미 담김" 상태(회색+체크)로 표시(#511) —
          // 안 띄우면 유저가 잊고 다시 물었을 때 칩이 왜 없는지 헷갈린다. 클릭 시 안내만.
