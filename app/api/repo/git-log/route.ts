...
// %H해시|%P부모들|%an작성자|%at시각|%D장식(refs)|%s제목. --all로 전 브랜치, -n 200 상한.
const ARGS = ["log", "--all", "--date-order", "-n", "200", "--pretty=format:%H|%P|%an|%at|%D|%s"];
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const { stdout } = await pexecFile("git", ARGS, { cwd: path, maxBuffer: 10_000_000, timeout: 8000 });
    return Response.json({ ok: true, isGit: true, log: stdout });
  } catch {
